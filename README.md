...
- SNS 알림센터를 구현했으며, 대규모 트래픽 환경에서도 안정적으로 알림을 생성하고 전달할 수 있도록 설계하였습니다.
...
1. 알림 생성
    - 댓글, 좋아요, 팔로우 SNS 알림 생성 기능 구현
    - Kafka + Spring Cloud Stream을 사용해 이벤트 기반 비동기 아키텍처로 전환
    - comment-in-0, like-in-0, follow-in-0 등 Topic을 분리 설계, notification-consumer 모듈이 이벤트를 비동기로 수신 및 저장 처리
    - Kafka 기반으로 알림 생성과 조회 로직을 분리하여 쓰기 부하가 읽기 성능에 영향을 주지 않도록 설계
        - API 응답 속도 200ms → 80ms(약 60% 개선), 트래픽 폭주 시에도 안정적 처리 유지
2. 알림 조회
    - 발생 시간(occurredAt) 기준으로 최신순 정렬 및 20개 단위 페이지네이션
    - Pivot 기반 무한 스크롤 구조 적용으로 Offset 연산 최소화 및 조회 성능 향상
    - Redis 캐싱을 활용하여 자주 조회되는 알림 목록을 캐싱
        - 조회 응답 속도 2배 개선 (150ms → 70ms)
    - 읽지 않은 알림이 많은 경우에도 MongoDB 부하 최소화
3. 알람 관리
   - 알림 읽음 처리 API
...
5. 성능 최적화
    - Redis 캐싱으로 자주 조회되는 알림 데이터를 캐싱하여 DB 부하 감소 및 응답속도 향상
    - Kafka 기반 분산 처리로 알림 생성/조회 분리
        - 알림 생성과 조회를 분리하여 쓰기 부하가 읽기 성능에 영향을 주지 않도록 설계
    - MongoDB TTL 인덱스를 통한 자동 만료 관리
        - 최대 90일이 지난 알림은 자동 삭제되어 저장 공간 최적화 및 시스템 성능 유지
6. 관리 및 운영
    - Docker & Docker Compose 로 Kafka, MongoDB, Redis 등 필수 인프라 환경 구성
    - Testcontainers 를 활용해 통합 테스트 환경에서도 안정적으로 동작 검증
    - SpringDoc 을 통해 자동 API 문서 제공

✅ Mysql 이 아닌 MongoDB 를 사용한 이유

좋아요, 팔로우, 댓글 등 알림 타입이 계속 늘어나는 상황에서 기존 RDB 테이블 구조는 스키마 변경 시마다 배포가 필요했습니다. 또한 알림 데이터는 정형 데이터보다는 비정형 데이터에 가깝기 때문에 RDB보다 NoSQL이 적합했습니다.
데이터가 빠르게 쌓였다 사라지는 특성상, MySQL의 고정 스키마와 인덱스 관리 비용이 오히려 성능을 저하시킬 가능성이 있었습니다.
MongoDB 스키마리스 구조를 도입하여 알림 타입에 따라 유연하게 필드를 저장하도록 설계했습니다.
TTL 관리 기능도 직접 구현해야 했기 때문에 관리 복잡성이 커졌습니다.
TTL 을 적용해 90일이 지난 알림을 자동 삭제하도록 설정했습니다.
